...
async function fetchSafeRoutes(start, end) {
    ...
    try {
        ...
        if (mainRoute) {
            // Green gets Calculated Score + Main Road Boost + USER ADJUSTMENT
            const greenAdj = getLocalScoreAdjustment(document.getElementById("start-loc").value, document.getElementById("dest-loc").value, 'r_green');
            const finalGreenScore = Math.min(100, Math.max(0, areaBaseScore + 5 + greenAdj));
        ...
        if (walkerRoute) {
            ...
            const yellowAdj = getLocalScoreAdjustment(document.getElementById("start-loc").value, document.getElementById("dest-loc").value, 'r_yellow');
            const finalYellowScore = Math.min(100, Math.max(0, Math.max(20, walkScore) + yellowAdj));
        ...
        if (shortcutRoute) {
            ...
            const redAdj = getLocalScoreAdjustment(document.getElementById("start-loc").value, document.getElementById("dest-loc").value, 'r_red');
            const finalRedScore = Math.min(100, Math.max(0, Math.max(10, riskyScore) + redAdj));
...
document.getElementById("rate-route-btn").addEventListener("click", () => {
    ...
    // Generate a simple key for this specific route journey
    const startLoc = document.getElementById("start-loc").value.trim();
    const destLoc = document.getElementById("dest-loc").value.trim();
    if (!startLoc || !destLoc) return;

    // Key format: rate_START_END_ROUTEID
    // We lowercase and remove spaces to make it more robust
    const key = `rate_${startLoc.toLowerCase().replace(/\s/g, '')}_${destLoc.toLowerCase().replace(/\s/g, '')}_${selectedRouteId}`;

    localStorage.setItem(key, rating);

    // Provide feedback
    alert(rating <= 2
        ? "⚠️ Safety concern recorded locally. This route's score will decrease for you."
        : "✅ Feedback saved! This route's score will increase for you."
    );

    // Optional: Immediately refresh routes to show new score?
    // For now, user will see it on next search.
});

// Helper to get adjustment
function getLocalScoreAdjustment(start, end, routeId) {
    if (!start || !end) return 0;
    const key = `rate_${start.toLowerCase().replace(/\s/g, '')}_${end.toLowerCase().replace(/\s/g, '')}_${routeId}`;
    const savedRating = localStorage.getItem(key);

    if (savedRating) {
        const r = parseInt(savedRating);
